...
public class MineSweeper {
	...
	public void run() {
		...
		while(sumLoop > 0) {
			...
			System.out.print("Tahmini satýr sayýsýný giriniz : ");
			int row = input.nextInt();
			System.out.print("Tahmini sütun sayýsýný giriniz : ");
			int col = input.nextInt();
			
			if(row < 0 && col < 0) {
				System.out.println("Satýr ve Sütun sayýsý negatif olamaz!");
				break;
			}else {
				...
				if(rowNumber > row && colNumber > col) {
					...
					System.out.println("\nYanlýþ satýr veya sütun deðeri girdiniz !");
					break;
				}
